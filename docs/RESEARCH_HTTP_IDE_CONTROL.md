...
限制：這四組都是用 `--runscript` 在啟動時執行腳本。從 Tools 選單啟動腳本的路徑我沒有量測到，因為 UI 自動化在 CODESYS 的 WinForms 選單上失敗，而且第二個 3.5.21.40 實例一開就跳出「有未儲存的專案資料可以復原」的對話框（指向使用者正開著的專案），我沒有回答它就把測試實例關掉了。兩種啟動方式的腳本都跑在 UI 執行緒上，經過同一個 ScriptEngine 執行器，我預期行為相同，但這是推測。手動驗證用的腳本 `tools/probe_ui_responsive.py` **已移除**：它在主執行緒上用 `system.delay(50)` 迴圈 45 秒（或用 `time.sleep(0.05)` 當對照），讓人在旁邊試著點選單和捲動編輯器，同時把自己所在的執行緒寫進 log。留著會誤導，因為 2026-09-05 用真實滑鼠量出來的結果是：`system.delay()` 迴圈期間選單本來就打不開（見本節後面的更正與 `WATCHER_CLI_PLAN.md` 第 14 節），所以那支腳本問的是一個已經有答案、而且答案跟它當初的結論相反的問題。現在要量點不點得動，用 `tools/probe_click_menu.py`，它從 IDE 外面送真實的滑鼠事件。
